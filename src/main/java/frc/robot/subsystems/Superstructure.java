...
//import frc.lib.util.drivers.RevRoboticsAirPressureSensor;
...
/*
    ...
    private SystemState handleHang() {
        ...
        mFeeder.setWantedState(Feeder.WantedState.IDLE);
        mHopper.setWantedState(Hopper.WantedState.IDLE);
        mShooter.setOpenLoop(-12.0); //run shooter backwards at full speed
    ...
    private double getShootingSetpointRpm(double range) {
        if (Constants.kUseFlywheelAutoAimPolynomial) { //currently true
            return Constants.kFlywheelAutoAimPolynomial.predict(range); //based on the data points, outputs correct rpm for range
        } else {
            return Constants.kFlywheelAutoAimMap.getInterpolated(new InterpolatingDouble(range)).value;
        }
    }

    //spin shooter to a set rpm and makes sure range and drive base angle are all good, also allows for tuning at certain rpms
    public synchronized boolean autoSpinShooter(boolean allow_shooting) {
        final double timestamp = Timer.getFPGATimestamp();
        final Optional<ShooterAimingParameters> aimOptional = RobotState.getInstance() //Get aim parameters from robot state
                .getAimingParameters();                                                 // Optional prevents null pointer exceptions 
        mLED.setWantedState(LED.WantedState.FIND_RANGE);                                //If the robot state is null, it's ok
        if (aimOptional.isPresent()) {
            final ShooterAimingParameters aim = aimOptional.get();
            double range = aim.getRange(); //distance robot needs to shoot
            final boolean range_valid = Constants.kIsShooterTuning || (range >= Constants.kShooterAbsoluteRangeFloor
                    && range <= Constants.kShooterAbsoluteRangeCeiling); //is the range between the threshold of minimum and max distances
            if (!range_valid) {
                range = Math.max(Constants.kShooterAbsoluteRangeFloor,
                        Math.min(Constants.kShooterAbsoluteRangeCeiling, range)); //if range outside of threshold, set it to the max or min
            }                                                                       //lol possibly over or under shoot
            if (!Constants.kIsShooterTuning) { //If we aren't tuning the shooter

                mLastGoalRange = range; //update range
                double setpoint = getShootingSetpointRpm(range); //using the polynomial to predict best rpm for a range
                if (allow_shooting && aim.getStability() >= Constants.kShooterMinTrackStability) { //did it specify to shoot when ready, and is it stable?
                    mShooter.setHoldWhenReady(setpoint); //if it's all good, hold when ready at the calculated rpm setpoint
                } else {
                    setpoint = getShootingSetpointRpm(Constants.kShooterOptimalRange); // if it isn't stable, set to optimal range
                    mShooter.setSpinUp(setpoint); //just spin it up to the calculated rpm based on best range
                }
                ...
                if (range < Constants.kShooterOptimalRangeFloor) {
                    ...
                    mLED.setRangeBlicking(false);        //if user too far or close, blinks range signal, if good, solid led?
                    mLED.setRangeLEDOn();

                    is_optimal_range = true;
                }

                SmartDashboard.putBoolean("optimal range", is_optimal_range); //allows driver to see in dash it is optimal
            } else {
                // We are shooter tuning find current RPM we are tuning for.
                mShooter.setHoldWhenReady(mCurrentTuningRpm); //set to tuning rpm
                mLastGoalRange = aimOptional.get().getRange(); //update range from target TODO: how do they correlate an RPM to a range
            }

            return range_valid && isOnTargetToShoot() //is on target checks if drive base on target and if shooter is on target
                    && (timestamp - aim.getLastSeenTimestamp()) < Constants.kMaxGoalTrackAge; //goal was seen at least recently - if everything good, continues and returns true
        } else if (Superstructure.getInstance().isShooting()) { //aimOptional is not present and it is shooting
            mLED.setRangeBlicking(true); //range not good
            ...
            if (mShooter.getSetpointRpm() < Constants.kShooterTuningRpmFloor) { 
                ...
                // re-appears.
                mShooter.setSpinUp(getShootingSetpointRpm(Constants.kDefaultShootingDistanceInches)); //don't know range, set it to default
            }
            return false; //false because it doesn't see the goal
        }
    }
